...
/*$fb = new Facebook\Facebook([
  'app_id' => '920898154689589',
  'app_secret' => '080950e6378486a361de44014533b441',
	...
	<main class="container">
		<article>
			...
			<div class="lower rules-voting">
				<div class="row">
					<?php
					$regx = "/https:\/\/www\.youtube\.com\/embed\/[\w-]*/i";
                    foreach($MovieList as $mkey => $mval){
						/*if (! isset($accessToken)) {
							$vote_url = $helper->getLoginUrl('http://www.taiwan235n.tw/moving/vote_action.php?id='.$mval['movie_list_id'], $permissions); 
						}else{
							$vote_url = "vote_action.php?id=".$mval['movie_list_id'];
						}*/
						$vote_url = 'http://www.taiwan235n.tw/moving/vote_action.php?id='.$mval['movie_list_id'];
						preg_match($regx,$mval['involve_youtube'],$youtube);
						$youtube_url = (isset($youtube[0])) ? $youtube[0] : "#";
						$no = intval($mval['movie_list_no']);
						//$no = ($PageNO * $_PageShowNum)+($mkey+1);
					?>
						...
						<div class="vedio-wrap">
							...
							<div class="details">
								...
								<div class="right">
									<div class="btn-group">
                                    	<?php
                                        if($now >= $start_time && $now <= $check_time){
											echo '<div class="btn voteBtn" href="'.$vote_url.'" data-id="'.$mval['movie_list_id'].'">投票</div>';
											
										}else{
											echo '<div class="btn" data-toggle="modal" data-target="#myModal">投票</div>';
										}
										?>
										
										<a class="btn fb btnShare" data-image="none.jpg" 
											data-title="<?=$mval['involve_movie']?>" data-desc="<?=$mval['involve_intro']?>" href="<?=$youtube_url?>">分享</a>
									</div>
									<div class="like">
