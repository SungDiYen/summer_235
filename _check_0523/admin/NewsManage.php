<?php
	...
	//初始值設定
	$_FileName       = "NewsManage.php";              //本檔案檔名
	$_Title          = "News";                    //本檔案Title
	$_MainTitleName  = "最新消息";                       //本檔案標題
	$_SubTitleName   = "最新消息";
	$_SessionTitle   = "News";                        //SessionTitle
	...
	$smarty->assign('uploadUrl',Upload_Url."news/");
	...
	$smarty->assign("_NowType",$_NowType);*/
	
	$smarty->assign("_NewsType",$_NewsType);
...
if ($_REQUEST['flag'] == 'edit')
{
	...
	if($id != ""){
  		...
  		$sqlstr = "Select * FROM `".Table_News."` WHERE `DelFlag` = '' and `news_id` = '".$id."'";
...
elseif ($_REQUEST['flag'] == 'editsave')
{
	...
	$sqlstr = "SELECT * FROM `".Table_News."` WHERE `news_id` = '".$id."'";
	
	$data['news_title']     = mysql_real_escape_string($_POST['news_title']);
	$data['news_date']      = mysql_real_escape_string($_POST['news_date']);
	//$data['news_quotation'] = mysql_real_escape_string($_POST['news_quotation']);
	$data['news_content']   = mysql_real_escape_string($_POST['news_content']);
	$data['sortnum']        = intval($_POST['sortnum']);
	$data['show_status']    = mysql_real_escape_string($_POST['show_mode']);

	//pic
	if($_FILES["news_pic"]["name"] != ""){
		$FileType = strtolower(substr($_FILES["news_pic"]["name"],strrpos($_FILES["news_pic"]["name"],'.')));
		if($FileType == '.jpg' || $FileType == '.jpeg' || $FileType == '.png' || $FileType == '.gif'){
			$FileName = "news-".date("ymdHis").$FileType;
			$data["news_pic"] = $FileName;			
			copy( $_FILES["news_pic"]['tmp_name'], Upload_Path."/news/".$FileName );	
			
		}
	}elseif($_POST["pic_Del"]=="*"){
		if($_POST['opic']!=""){
			unlink(Upload_Path."/news/".$_POST['opic']);
		}
		$data["news_pic"]="";
	}
	//內文pic
	/*if($_FILES["news_desc_pic"]["name"] != ""){
		$FileType = strtolower(substr($_FILES["news_desc_pic"]["name"],strrpos($_FILES["news_desc_pic"]["name"],'.')));
		if($FileType == '.jpg' || $FileType == '.jpeg' || $FileType == '.png' || $FileType == '.gif'){
			$FileName = "newsd-".date("ymdHis").$FileType;
			$data["news_desc_pic"] = $FileName;			
			copy( $_FILES["news_desc_pic"]['tmp_name'], Upload_Path."/news/".$FileName );	
	...
	if($id != "" && $g_db->numRows( $g_db->query($sqlstr) ) > 0){
		...
		$g_db->update( Table_News, $data, "news_id", $id );
...
elseif ($_REQUEST['flag'] == 'delete')
{
	...
	if(is_array($ids)){
		foreach($ids as $key => $val){
			...
			$g_db->update( Table_News, $data, "news_id", $val );
		}
	}else{
		$data['DelFlag'] = "Y";
	
		$g_db->update( Table_News, $data, "news_id", $ids );
	}
	header("Location: ".$_FileName);
...
else
{
	...
	if(!empty($_SESSION[$_SessionTitle.'Name'])){
		$WhereStr .= " and (news_title like '%".mysql_real_escape_string($_SESSION[$_SessionTitle.'Name'])."%' or news_content like '%".mysql_real_escape_string($_SESSION[$_SessionTitle.'Name'])."%')";
	...
	if(!empty($_SESSION[$_SessionTitle.'TStatus'])){
		$WhereStr .= " and show_titbits = '".mysql_real_escape_string($_SESSION[$_SessionTitle.'TStatus'])."'";
		$smarty->assign("SearchTStatus",$_SESSION[$_SessionTitle.'TStatus']);
	}
	...
	switch($_SESSION[$_SessionTitle.'SortBy']){
		case "pdate":
			$OrderStr = " news_date ".$_SESSION[$_SessionTitle.'Sort'].",";
		break;
	}
	...
	//搜尋資料
    $sqlstr = "Select * FROM `".Table_News."` WHERE `DelFlag` = ''  ".$WhereStr;
