<?php
	...
	//初始值設定
	$_FileName       = "InvolveManage.php";              //本檔案檔名
	$_Title          = "Involve";                    //本檔案Title
	$_MainTitleName  = "活動報名管理";                       //本檔案標題
	$_SubTitleName   = "活動報名管理";
	$_SessionTitle   = "Involve";                        //SessionTitle
...
if ($_REQUEST['flag'] == 'edit')
{
	...
	if($id != ""){
  		...
  		$sqlstr = "Select * FROM `".Table_Movie."` WHERE `DelFlag` = '' and `movie_list_id` = '".$id."'";
	...
	$smarty->display("InvolveEdit.tpl");
...
elseif ($_REQUEST['flag'] == 'editsave')
{
	...
	$sqlstr = "SELECT * FROM `".Table_Movie."` WHERE `movie_list_id` = '".$id."'";
	
	$data['involve_name']     = StrFilter($involve_name);
	$data['involve_tel']      = StrFilter($involve_tel);
	$data['involve_email']    = StrFilter($involve_email);
	$data['involve_movie']    = StrFilter($involve_movie);
	$data['involve_zip']      = StrFilter($zipcode);
	$data['involve_county']   = StrFilter($county);
	$data['involve_district'] = StrFilter($district);
	$data['involve_address']  = StrFilter($involve_address);
	$data['involve_intro']    = StrFilter($involve_intro);
	$data['involve_school']      = StrFilter($involve_school);
	$data['involve_department']  = StrFilter($involve_department);
	$data['involve_grade']       = StrFilter($involve_grade);
...
elseif ($_REQUEST['flag'] == 'delete')
{
	...
	if(is_array($ids)){
		foreach($ids as $key => $val){
			...
			$g_db->update( Table_Movie, $data, "movie_list_id", $val );
		}
	}else{
		$data['DelFlag'] = "Y";
	
		$g_db->update( Table_Movie, $data, "movie_list_id", $ids );
	}
	header("Location: ".$_FileName);
...
else
{
	...
	if(!empty($_SESSION[$_SessionTitle.'Name'])){
		$WhereStr .= " and (involve_name like '%".mysql_real_escape_string($_SESSION[$_SessionTitle.'Name'])."%' or involve_movie like '%".mysql_real_escape_string($_SESSION[$_SessionTitle.'Name'])."%')";
	...
	switch($_SESSION[$_SessionTitle.'SortBy']){
		case "pdate":
			$OrderStr = " create_time ".$_SESSION[$_SessionTitle.'Sort'].",";
		break;
	}
	...
	//搜尋資料
    $sqlstr = "Select * FROM `".Table_Movie."` WHERE `DelFlag` = ''  ".$WhereStr;
	...
	//顯示網頁
    $smarty->display("InvolveList.tpl");
