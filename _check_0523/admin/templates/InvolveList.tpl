                	...
                	<div class="table-responsive">
                        ...
                        <table class="table table-striped table-bordered table-hover dataTable no-footer" id="dataTables-example" aria-describedby="dataTables-example_info">
                            <tr>
                                ...
                                <th>email</th>
                                <th>作品名稱</th>
                                <th width="100px">新增日期 <a class="fa {if $sortby==pdate}fa-sort-{$sorticon}{else}fa-sort{/if}" href="{$_FileName}?flag=sort&sortby=pdate"></a></th>
                                <th width="80px">是否顯示</th>
                                <th width="60px">編輯</th>
                            </tr>
                            {foreach from = $DataList item = obj name = DataList}
                            <tr>
                                <td><input type="checkbox" name="ids[]" value="{$obj.movie_list_id}" /></td>
                                <td>{$obj.movie_list_no}</td>
                                <td>
                                    {$obj.involve_name}
                                </td>
                                <td>{$obj.involve_tel}</td>
                                <td>{$obj.involve_email}</td>
                                <td>{$obj.involve_movie}</td>
                                <td>{$obj.create_time}</td>
                                <td>{$obj.show_status}</td>
                                <td>
                                    <a href="{$_FileName}?flag=edit&id={$obj.movie_list_id}" title="修改"><i class="fa fa-pencil"></i></a>
                                    <a href="javascript:void(0)" onclick="if(confirm('確認刪除?'))location.href='{$FileName}?flag=delete&ids={$obj.movie_list_id}'" title="刪除"><i class="fa fa-trash-o"></i></a> 
                                </td>
                            </tr>
                           {/foreach}
                        </table>
